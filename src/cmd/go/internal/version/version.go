...
If no files are named on the command line, moxie version prints its own
version information.

If a directory is named, moxie version walks that directory, recursively,
looking for recognized Moxie binaries and reporting their versions.
By default, moxie version does not report unrecognized files found
during a directory scan. The -v flag causes it to report unrecognized files.

The -m flag causes moxie version to print each file's embedded
module version information, when available. In the output, the module
information consists of multiple lines following the version line, each
indented by a leading tab character.

The -json flag is similar to -m but outputs the runtime/debug.BuildInfo in JSON format.
If flag -json is specified without -m, moxie version reports an error.

See also: moxie doc runtime/debug.BuildInfo.
`,
}
...
func runVersion(ctx context.Context, cmd *base.Command, args []string) {
	if len(args) == 0 {
		...
		if argOnlyFlag != "" {
			fmt.Fprintf(os.Stderr, "moxie: 'moxie version' only accepts %s flag with arguments\n", argOnlyFlag)
			base.SetExitStatus(2)
			return
		}
		v := runtime.Version()
		if gover.TestVersion != "" {
			v = gover.TestVersion + " (TESTMOXIE_VERSION)"
		}
	...
	if !*versionM && *versionJson {
		fmt.Fprintf(os.Stderr, "moxie: 'moxie version' with -json flag requires -m flag\n")
		base.SetExitStatus(2)
		return
	}
